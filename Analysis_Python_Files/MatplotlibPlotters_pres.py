...
def Survival(fileID, atomLocs, **TransferArgs):
    ...
    :return: see Transfer()
    """
    return Transfer(fileID, tao.getStandardSurvivalOptions(atomLocs), **TransferArgs)
...
def Transfer( fileNumber, anaylsisOpts, x_label=None, showgrid=False, round_x=False,show=True, legendOption=None, fitModules=[None], saveImage=False, 
    ...
    xlabels = [x_label,keyName,'Picture #','','']
    ylabels = [typeName,"Initial Pop %", "Camera Signal",'','']
    titles = [None, "Initial Pop: Avg$ = " + str(misc.round_sig(np.mean(arr(initPopulation)))) + '$',
    ...
    majorYTicks = [np.arange(0,1.1,0.1),np.arange(0,1.1,0.2),np.linspace(min(pic1Data[0]),max(pic1Data[0]),5),[],[]]
    minorYTicks = [np.arange(0,1.1,0.05),np.arange(0,1.1,0.1),np.linspace(min(pic1Data[0]),max(pic1Data[0]),10),[],[]]
    if len(key.shape) == 1 and round_x == True:
#           xtickKey = [round(num) for num in np.linspace(min(key),max(key),len(key))]
        xtickKey = [ round(num,1) for num in key] if len(key) < 30 else np.linspace(min(key),max(key),10)
#         xtickKey = key[0::3] if len(key) < 30 else np.linspace(min(key),max(key),10)
    else:
        xtickKey = key if len(key) < 30 else np.linspace(min(key),max(key),10)
    if len(key.shape) == 2:
        xtickKey = key[:,0] if len(key[:,0]) < 30 else np.linspace(min(key[:,0]),max(key[:,0]),10)
...
#         subplt.set_yticks(yTickMin, minor=True)      
        ...
        subplt.tick_params(direction='in', length=6, width=2, colors='k',
               grid_color='r', grid_alpha=0.5)
        rotateTicks(subplt)
        if showgrid == True:
            subplt.grid(grid, color='#909090', which='Major', linewidth=.5)
            subplt.grid(grid, color='#909090', which='Minor', linewidth=.5)
        else:
            subplt.grid(grid, color='w', which='Major', linewidth=.5)
            subplt.grid(grid, color='w', which='Minor', linewidth=.5)            
        rc('axes', linewidth=2)
...
#             item.set_fontweight('bold')
    ...
    for dataSetInc, (atomLoc, fit, module, color, edgecolor) in enumerate(zip(range(analysisOpts.numDataSets()), fits, fitModules, colors, dataEdgeColors)):
        ...
        mainPlot.errorbar ( key, transferData[dataSetInc], yerr=unevenErrs, color=color, ls='',
                           ...
                           alpha=0.3 if plotAvg else 0.9, marker=markers[dataSetInc%len(markers)], markersize=15) #markerEdgeColor=edgecolor, markerEdgeWidth=2
    ...
    mainPlot.xaxis.set_label_coords(.5, -0.1)
    
    if legendOption:
        mainPlot.legend(loc="upper right", bbox_to_anchor=(1, 1.13), fancybox=True, 
                        ncol = 4 if longLegend else 10, prop={'size': 18}, frameon=False)
    mainPlot.yaxis.set_label_coords(-.05, .5)  
    ...
    if saveImage:
        plt.savefig('test_skp.png',bbox_inches='tight', format='png', dpi=1200)     
    ...
    return returnObj
def PlotTwoDataSets(FileID1,FileID2,atomlocs,x_label = None, color1='tab:blue',color2='tab:green'):
    dataset1 = ta.standardTransferAnalysis(FileID1, tao.getStandardLoadingOptions(atomlocs));
    ...
    dataset2 = ta.standardTransferAnalysis(FileID2, tao.getStandardLoadingOptions(atomlocs));
    ...
    ax.errorbar(key1,avgTferData1, yerr=err1,ls='none',ecolor=color1,marker = 'o',markerfacecolor=color1,markeredgecolor='k',capsize=5)
    p = np.poly1d(np.polyfit(key1, avgTferData1, 4))
    plt.plot(t, p(t), '-',color=color1,label=' 5')

    ax.errorbar(key2,avgTferData2, yerr=err2,ls='none',ecolor=color2,marker = 'o',markerfacecolor=color2,markeredgecolor='k',capsize=5)
    p = np.poly1d(np.polyfit(key2, avgTferData2, 4))
    plt.plot(t, p(t), '-',color=color2,label=' 0')
    plt.rcParams["axes.linewidth"] = 1.5
    plt.legend(title="RF Power",loc='lower right')
    plt.ylim(0,1)
    plt.ylabel('survival')
    plt.xlabel(x_label)
    plt.rcParams["axes.linewidth"] = 1.5
...
def Loading(fileID, atomLocs, **TransferArgs):
    ...
    """
    return Transfer(fileID, tao.getStandardLoadingOptions(atomLocs), **TransferArgs)
...
def collision_detuning(fid,manualThresholdValue = 60):
    dataset1 = ta.standardTransferAnalysis(fid, loadTwoOpt, tOptions = [to.ThresholdOptions(manualThreshold = True, manualThresholdValue = 60)]);
    (tferAtomsVarAvg, tferAtomsVarErrs, loadAtomsVarAvg, initPicCounts, keyName, key1, repetitions, initThresholds, 
                fits, avgTferData1, avgTferErr1, avgFit, avgPics, genAvgs, genErrs, tferVarAvg, tferVarErr, initAtomImages, 
                tferAtomImages, tferPicCounts, tferThresholds, fitModules, basicInfoStr, ensembleHits, tOptions, analysisOpts,
                tferAtomsPs, tferAtomsPs, tferList, isAnnotated, hmm) = dataset1
    ...
    dataset2 = ta.standardTransferAnalysis(fid, loadOneOpt, tOptions = [to.ThresholdOptions(manualThreshold = True, manualThresholdValue = 60)]);
    (tferAtomsVarAvg, tferAtomsVarErrs, loadAtomsVarAvg, initPicCounts, keyName, key2, repetitions, initThresholds, 
                fits, avgTferData2, avgTferErr2, avgFit, avgPics, genAvgs, genErrs, tferVarAvg, tferVarErr, initAtomImages, 
                tferAtomImages, tferPicCounts, tferThresholds, fitModules, basicInfoStr, ensembleHits, tOptions, analysisOpts,
                tferAtomsPs, tferAtomsPs, tferList, isAnnotated, hmm) = dataset2
