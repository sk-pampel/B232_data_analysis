...
def fitManyGaussianImagePinhole(im, numGauss, neighborhood_size=20, threshold=1, direct=True, widthGuess=1):
    """
    Maxima finding is based on the answer to this question:
    https://stackoverflow.com/questions/9111711/get-coordinates-of-local-maxima-in-2d-array-above-certain-value
    """
    maximaLocs = findImageMaxima(im, neighborhood_size=neighborhood_size, threshold=threshold)
#     print(maximaLocs[1][0]-1,maximaLocs[1][0]+1,1)
    print(maximaLocs)
    if len(maximaLocs) != numGauss:
        raise ValueError("ERROR: didn't find the right number of maxima!")
    guess = [min(im.flatten())]
    ...
    ax2.imshow(zpts_fit_norm)
    ax2.set_yticks([])
    ax2.set_title('2D Gaussian Fit')
    fit_diff_norm = im_norm-zpts_fit_norm

    
    ax3.imshow(fit_diff_norm)
    ax3.set_yticks([])
    ax3.set_title('2D Residuals')
    
#     res_range = np.arange(maximaLocs[1][0]-50,maximaLocs[1][0]+50,1)
#     ax4.plot(res_range,(im_norm[maximaLocs[1][1]][np.amin(res_range):np.amax(res_range)+1]))
#     ax4.plot(res_range,(zpts_fit_norm[maximaLocs[1][1]][np.amin(res_range):np.amax(res_range)+1]))
#     ax4.set_title('1D Gaussian Fit')
#     ax4.set_xticks([])
#     ax5.plot(figsize=(20,10))
#     ax5.plot(res_range,(fit_diff_norm[maximaLocs[1][1]][np.amin(res_range):np.amax(res_range)+1])/np.amax(im))
#     ax5.set_title('1D Residuals') 
#     plt.colorbar(ax1.imshow(im_norm),ax=ax1,fraction=0.046, pad=0.04)
#     plt.colorbar(ax2.imshow(zpts_fit_norm),ax=ax2,fraction=0.046, pad=0.04)
#     plt.colorbar(ax3.imshow(fit_diff_norm,vmin=-.1, vmax=.1),ax=ax3,fraction=0.046, pad=0.04)
  
    ### for 1 pinhole
    res_range = np.arange(maximaLocs[0][0]-50,maximaLocs[0][0]+50,1)
    ax4.plot(res_range,(im_norm[maximaLocs[0][0]][np.amin(res_range):np.amax(res_range)+1]))
    ax4.plot(res_range,(zpts_fit_norm[maximaLocs[0][0]][np.amin(res_range):np.amax(res_range)+1]))
    ax4.set_title('1D Gaussian Fit')
    ax4.set_xticks([])
    ax5.plot(figsize=(20,10))
    ax5.plot(res_range,(fit_diff_norm[maximaLocs[0][0]][np.amin(res_range):np.amax(res_range)+1])/np.amax(im))
    ax5.set_title('1D Residuals') 
    plt.colorbar(ax1.imshow(im_norm),ax=ax1,fraction=0.046, pad=0.04)
    plt.colorbar(ax2.imshow(zpts_fit_norm),ax=ax2,fraction=0.046, pad=0.04)
    plt.colorbar(ax3.imshow(fit_diff_norm,vmin=-.1, vmax=.1),ax=ax3,fraction=0.046, pad=0.04)
    
    return optParam,im_norm,zpts_fit_norm,fit_diff_norm

def findImageMaximaCoords(im, numGauss, neighborhood_size=20, threshold=1, direct=True, widthGuess=1):
    maximaCoords = findImageMaxima(im, neighborhood_size=neighborhood_size, threshold=threshold);
    if len(maximaCoords) != numGauss:
        raise ValueError("ERROR: didn't find the right number of maxima!")
    return maximaCoords
...
def extract_NA_2pinholes(im, numGauss, neighborhood_size=20, threshold=1, direct=True, widthGuess=1,wavelength=0.852,pixel_factor=7):
    """
    Maxima finding is based on the answer to this question:
    https://stackoverflow.com/questions/9111711/get-coordinates-of-local-maxima-in-2d-array-above-certain-value
    """
    ...
    ######### Right pinhole
    #### Airy NA 
    xslice_2 = [int(optParam[7])-peak_offset,int(optParam[7])+peak_offset]; 
    yslice_2 = [int(optParam[8])-peak_offset,int(optParam[8])+peak_offset];
    XX_2,YY_2 = np.meshgrid(rr[xslice_2[0]:xslice_2[1]],rr[yslice_2[0]:yslice_2[1]])
    amp_2 = np.max(im[yslice_2[0]:yslice_2[1]])
    im_2 = im[yslice_2[0]:yslice_2[1], xslice_2[0]:xslice_2[1]]
    NA_guess = 0.5
    offset_2 = np.min(im[yslice_2[0]:yslice_2[1]])
    x1_2,y1_2 = rr[int(optParam[7])]+0.1,rr[int(optParam[8])]+0.1
    popt2A_2, pcov2A_2 = (curve_fit(oneAiry,(XX_2,YY_2),
                           im[yslice_2[0]:yslice_2[1], xslice_2[0]:xslice_2[1]].ravel(),
                                p0 = (x1_2,y1_2,NA_guess,offset_2,amp_2)))
    NA_airy_2 = np.sqrt(popt2A_2[2]**2/(1-popt2A_2[2]**2)) 
    x_air_2 = ufloat(popt2A_2[2], np.sqrt(pcov2A_2.diagonal())[2]*1.95)
    NA_airy_unc_2 = usqrt(x_air_2**2/(1-x_air_2**2)) 

    ## Gaussian NA
    x0_2,y0_2 = rr[int(optParam[7])]+0.1,rr[int(optParam[8])]+0.1
    offset_2 = np.min(im[yslice_2[0]:yslice_2[1]])
    theta = np.pi/2
    sigma_y_2,sigma_x_2 = np.std(XX_2), np.std(YY_2)
    popt2G_2, pcov2G_2 = (curve_fit(twoD_Gauss,(XX_2,YY_2), im[yslice_2[0]:yslice_2[1], 
                    xslice_2[0]:xslice_2[1]].ravel(),p0 = (amp_2,x0_2,y0_2,sigma_y_2,sigma_x_2,theta,offset_2)))

    NA_gauss_2 = np.sqrt(popt2G_2[3]**2/(1-popt2G_2[3]**2))
    x_2DG_2 = ufloat(popt2G_2[3], np.sqrt(pcov2G_2.diagonal())[3]*1.95)
    NA_gauss_unc_2 = usqrt(x_2DG_2**2/(1-x_2DG_2**2))

    average_NA_2 = (NA_gauss_unc_2+NA_airy_unc_2)/2
    
    ### for displaying airy fit and residuals
    ### pinhole 1
    # Create x and y indices
    x_1D_airy = np.linspace(1, 80, 80)
    y_1D_airy = np.linspace(1, 80, 80)
    xx_1D_airy, yy_1D_airy = np.meshgrid(x_1D_airy, y_1D_airy)

    # define coordinates of peak 
    x1_1D_airy = np.amax(x_1D_airy)/2+0.1
    y1_1D_airy = np.amax(y_1D_airy)/2+0.1

    # define pinhole image data and normalize
    im2_airy_1 = im[int(optParam[3]-40):int(optParam[3]+40),int(optParam[2]-40):int(optParam[2]+40)]
    im2_airy_norm_1 = im2_airy_1/np.amax(im2_airy_1)

    # Define Airy function arguments
    NA_guess = 0.5
    offset_airy_1 = np.amin(im2_airy_norm_1)
    amp_airy_1 = np.amax(im2_airy_norm_1)
    np.seterr(invalid='ignore') 

    # Generate fit and create 2D array
    popt2A_airy_1, pcov2A_airy_1 = (curve_fit(oneAiry,(xx_1D_airy,yy_1D_airy),im2_airy_1.ravel(), p0 = (x1_1D_airy,y1_1D_airy,NA_guess,offset_airy_1,amp_airy_1)))
    airy_fit_1 = oneAiry((xx_1D_airy, yy_1D_airy), *popt2A_airy_1).reshape(80,80)
    airy_fit_norm_1 = airy_fit_1/np.max(airy_fit_1)

    res_range_airy_1 = np.arange(0,len(im2_airy_1),1)
    airy_res_1 = im2_airy_norm_1[int(len(im2_airy_1)/2)][0:len(im2_airy_1)]-airy_fit_norm_1[int(len(im2_airy_1)/2)][0:len(im2_airy_1)]
   
    #### pinhole 2
    im2_airy_2 = im[int(optParam[8]-40):int(optParam[8]+40),int(optParam[7]-40):int(optParam[7]+40)]
    im2_airy_norm_2 = im2_airy_2/np.amax(im2_airy_2)
    
    # Define Airy function arguments
    offset_airy_2 = np.amin(im2_airy_norm_1)
    amp_airy_2 = np.amax(im2_airy_norm_1)
    np.seterr(invalid='ignore') 

    # Generate fit and create 2D array
    popt2A_airy_2, pcov2A_airy_2 = (curve_fit(oneAiry,(xx_1D_airy,yy_1D_airy),im2_airy_2.ravel(), p0 = (x1_1D_airy,y1_1D_airy,NA_guess,offset_airy_2,amp_airy_2)))
    airy_fit_2 = oneAiry((xx_1D_airy, yy_1D_airy), *popt2A_airy_2).reshape(80,80)
    airy_fit_norm_2 = airy_fit_2/np.max(airy_fit_2)

    res_range_airy_2 = np.arange(0,len(im2_airy_2),1)
    ...
    print('\033[1m' +'Average NA (pinhole 2) ='+'\033[0m',average_NA_2)
    ...
    ax4 = plt.subplot(2,5,7)
    ax5 = plt.subplot(2,5,3)
    ax6 = plt.subplot(2,5,8)
    ax7 = plt.subplot(2,5,4)
    ax8 = plt.subplot(2,5,9)
    ax9 = plt.subplot(2,5,5)
    ax10 = plt.subplot(2,5,10)
    axes = [ax1,ax2, ax3, ax4, ax5, ax6,ax7,ax8,ax9,ax10]
#     plt.rcParams["figure.autolayout"] = True
    ax1.imshow(pinhole_1_im_norm)
    ax1.set_title('Pinhole 1')
    ax2.imshow(pinhole_2_im_norm)
    ax2.set_title('Pinhole 2')
    ax3.plot(pinhole_1_xrange,(im_norm[maximaLocs[0][1]][np.amin(pinhole_1_xrange):np.amax(pinhole_1_xrange)+1]))
    ax3.plot(pinhole_1_xrange,(pinhole_1_xfit_gauss))
    ax3.set_title('Gaussian Fit')
    ax3.set_xticks([])
    ax4.plot(pinhole_2_xrange,(im_norm[maximaLocs[1][1]][np.amin(pinhole_2_xrange):np.amax(pinhole_2_xrange)+1]))
    ax4.plot(pinhole_2_xrange,(pinhole_2_xfit_gauss))
    ax4.set_title('Gaussian Fit')
    ax4.set_xticks([])
    ax5.plot(pinhole_1_xrange,(fit_diff_norm[maximaLocs[0][1]][np.amin(pinhole_1_xrange):np.amax(pinhole_1_xrange)+1])/np.amax(im))
    ax5.set_title('Gaussian Fit Residuals')
    ax5.set_xticks([])    
    ax6.plot(pinhole_2_xrange,(fit_diff_norm[maximaLocs[1][1]][np.amin(pinhole_2_xrange):np.amax(pinhole_2_xrange)+1])/np.amax(im))
    ax6.set_title('Gaussian Fit Residuals') 
    ax6.set_xticks([])
    
    ax7.plot(res_range_airy_1,im2_airy_norm_1[int(len(im2_airy_norm_1)/2)][0:len(im2_airy_norm_1)])
    ax7.plot(res_range_airy_1,airy_fit_norm_1[int(len(im2_airy_norm_1)/2)][0:len(im2_airy_norm_1)])
    ax7.set_title('Airy Fit') 
    ax7.set_xticks([])    
    ax8.plot(res_range_airy_2,im2_airy_norm_2[int(len(im2_airy_norm_2)/2)][0:len(im2_airy_norm_2)])
    ax8.plot(res_range_airy_2,airy_fit_norm_1[int(len(im2_airy_norm_2)/2)][0:len(im2_airy_norm_2)])
    ax8.set_title('Airy Fit') 
    ax8.set_xticks([])     
    ax9.plot(res_range_airy_1, airy_res_1)
    ax9.set_title('Airy Fit Residuals') 
    ax9.set_xticks([])
    ax10.plot(res_range_airy_2, airy_res_2)
    ax10.set_title('Airy Fit Residuals') 
    ax10.set_xticks([])
    plt.colorbar(ax1.imshow(pinhole_1_im_norm),ax=ax1,fraction=0.046, pad=0.04)
    plt.colorbar(ax2.imshow(pinhole_1_im_norm),ax=ax2,fraction=0.046, pad=0.04)
...
def fitPic(picture, showFit=True, guessSigma_x=1, guessSigma_y=1, guess_x=None, guess_y=None, fitF=gaussian_2d.f_notheta, guessOffset=None, extraGuess=None): #
    """
       guessSigma_x=1, guessSigma_y=1, Fit an individual picture with a 2d gaussian, and fit the horizontal and vertical averages with 1d gaussians
    """
...
#############  Release and recapture monte carlo fit
