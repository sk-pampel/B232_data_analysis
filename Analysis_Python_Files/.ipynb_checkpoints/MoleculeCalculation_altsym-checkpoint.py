...
def create_unsym_BO_HF_basis(L_vals, S_vals, i_vals):
    unsym_BO_HF_basis = []
    for chi in [1,-1]: 
        for L in L_vals: 
                            ...
                            for i_b in i_vals:
                                for I_ in np.arange(abs(i_b-i_a), i_b+i_a+1,1):
                                    for Iota in np.arange(-I_,I_+1,1):  
                                        chi_label = 'g' if chi==1 else 'u' 
                                        state = {'L':L, 'Lambda': Lambda, 'S':S,
                                                 'Sigma':Sigma, 'Omega':Sigma+Lambda,'chi': chi_label, 'I':int(I_),'Iota':int(Iota),'Phi':int(Sigma+Lambda+Iota)}
                                        unsym_BO_HF_basis.append(state)
    return unsym_BO_HF_basis    


def create_sym_BO_HF_states(l_a,l_b,s_a,s_b,i_a,i_b):
    sym_BO_states = []
    L=l_a+l_b
    for Lmbda in [0,1]:
        for S in [0,1]:
                    ...
                    if Lmbda == 1 and Sigma == -1: # 3Pi states --> Omega=0
                        for sigma_v in [-1,1]:
                            bostates = {'L':int(L),'|Lambda|':abs(Lmbda),'S':int(S),'|Sigma|':int(abs(Sigma)),
                                                   '|Omega|':int(abs(Omega)),'chi':chi, 'sigma_v':sigma_v }  
                            sym_BO_states.append(bostates)
                        # print(sym_BO_states)
                    elif Sigma>=0:      
                        sigma_v = (-1)**(Lmbda+(S)-Sigma)
                        bostates = {'L':int(l_a+l_b),'|Lambda|':abs(Lmbda),'S':int(S),'|Sigma|':int(abs(Sigma)),
                                                   '|Omega|':int(abs(Omega)),'chi':chi, 'sigma_v':int(sigma_v)}            
                        sym_BO_states.append(bostates)
        ...
        for I_ in IVals:
            for Iota in np.arange(-I_, I_+1,1):
                ...
                for Omega in [-state['|Omega|'], state['|Omega|']]:
                    ...
                    mostUpdates = {"|Phi|":int(abs(Phi)),'I':int(I_), '|Iota|':int(abs(Iota))}
                    ...
                    if Omega != 0 and Phi == 0:
                        ...
                        for sigma_HFS in [-1,1]:
                            newState = copy.copy(state)
                            newState.update(mostUpdates)
                            newState.update({'sigma_HFS': int(sigma_HFS)})
                            if newState not in expandedBasis:
                                expandedBasis.append(newState)
                    else:
                        newState = copy.copy(state)
                        newState.update(mostUpdates)
                        newState.update({ 'sigma_HFS':int(state['sigma_v']*(-1)**(I_-Iota) )})
                        if newState not in expandedBasis:
                            expandedBasis.append(newState)
...
def get_phi_bases(sym_BO_HF_states,unsym_BO_HF_basis):
    phi=0
    ...
    for totalSpatialParity in totalSpatialParityIncluded:
        ...
        for state in sym_BO_HF_states:
            chi = 0 if state['chi'] == 'g' else 1
            if math.isclose(abs(state['|Phi|']),phi):
                if (-1)**(state['I']+chi) == totalSpatialParityVal:
                    sigma_HFS = '2' if state['sigma_HFS'] == 1 else '1'
                    symmetryLabel = str(phi) + totalSpatialParity + sigma_HFS
    ...
    for phi in phiIncluded:
        for totalSpatialParity in totalSpatialParityIncluded:
            totalSpatialParityVal = 1 if totalSpatialParity == 'a' else -1
            for state in unsym_BO_HF_basis:
                chi = 0 if state['chi'] == 'g' else 1
                if math.isclose(abs(state['Phi']),phi):
                    if (-1)**(state['I']+chi+1) == totalSpatialParityVal:
                        label = str(phi) + totalSpatialParity
                        # print(totalSpatialParity,label)

                        if label not in mostlySymmetrizedBases:
                            mostlySymmetrizedBases[label] = []
                        mostlySymmetrizedBases[label].append(state)                    
    
    return symmetrizedBases,mostlySymmetrizedBases
...
def caseASymHfsToMostlySym(state, mostlySymBasis, indexes=False):
    ...
    if state['|Omega|'] == 0 and state['|Iota|'] == 0:
        ...
        for key in state.keys():
            if key in ["sigma_v", "sigma_HFS", "|Lambda|", "|Sigma|", "|Omega|"]:
                pass
            elif key[0] != "|":
        ...
        # Im confused about why this seems to need to involve sigma_v to work.
        
        sign = '+' if state['sigma_HFS']*state['sigma_v']*(-1)**(state['I']-state['|Iota|'])==1 else '-'
        if indexes:
...
def caseASymFsToMostlySym(state, mostlySymBasis, indexes=False):
    ...
    if state['|Lambda|'] == 0 and state['|Sigma|'] == 0:
        ...
        if (state['|Lambda|'] + state['S'] - state['|Sigma|'])==0 or (state['|Lambda|'] + state['S'] -state['|Sigma|']) == 2 :

            sign = '+' if state['sigma_v'] == 1 else '-'
        else:
            sign = '-' if state['sigma_v'] == 1 else '+'
...
def create_H_HFS_noF0(hfs_basis, E_5P12_F1F2_splitting, E_HFS_5S12_F1F2_splitting, F3E, F2E, F1E):
    H_HFS = np.zeros((len(hfs_basis),len(hfs_basis)))
    name_pref = ['f','m_f','j','i']
    for s1num, state1 in enumerate(hfs_basis):
        qNums = [0 for _ in range(8)]
        ...
        f_a,mf_a,j_a,i_a,f_b,mf_b,j_b,i_b = qNums
        A_a_HFS = E_5P12_F1F2_splitting if state1["l_a"] == 1 else E_HFS_5S12_F1F2_splitting
        E1 = A_a_HFS/2 * (f_a*(f_a+1)-j_a*(j_a+1)-i_a*(i_a+1)) if state1["j_a"] != 3/2 else (F3E if f_a==3 else (F2E if f_a==2 else (F1E)))
        A_b_HFS = E_5P12_F1F2_splitting if state1["l_b"] == 1 else E_HFS_5S12_F1F2_splitting
        E2 = A_b_HFS/2 * (f_b*(f_b+1)-j_b*(j_b+1)-i_b*(i_b+1)) if state1["j_b"] != 3/2 else (F3E if f_b==3 else (F2E if f_b==2 else (F1E)))
        H_HFS[s1num,s1num] = E1 + E2
    return H_HFS
...
def get_H_BO(C3, Rv, bo_basis):
    ...
    for num, state in enumerate(bo_basis):
        chi = 0 if state["chi"]=="g" else 1
        L_ = state["L"]
        H_BO[num,num] = (2*(C3/Rv**3)*float((-1))**(state[lambdaKey]+state['S']+chi)
                        /(math.factorial(1+state[lambdaKey])*math.factorial(1-state[lambdaKey])))
    return H_BO
